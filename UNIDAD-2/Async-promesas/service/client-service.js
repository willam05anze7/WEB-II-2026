...
// //---------con sqlserver
// const api_base_url = 'http://localhost:3000/clientes';
...
// const cliente = (id) => {
//     return fetch(`${api_base_url}/${id}`).then(respuesta => respuesta.json());
// };
...
//--------trabajandno con express
const BASE_URL = "http://localhost:3000";

const clienteService = {
    listar_clientes: async () => {
        const res = await fetch(`${BASE_URL}/_cliente`);
        return res.json();
    },
    cliente: async (id) => {
        const res = await fetch(`${BASE_URL}/_cliente/${id}`);
        return res.json();
    },
    crear_cliente: async (nombre, email) => {
        const res = await fetch(`${BASE_URL}/_cliente`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ nombre, email })
    ...
    actualizar_cliente: async (nombre, email, id) => {
        const res = await fetch(`${BASE_URL}/_cliente/${id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ nombre, email })
