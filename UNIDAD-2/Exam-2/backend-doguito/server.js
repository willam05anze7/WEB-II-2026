...
app.post('/cliente', async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO cliente (id_cliente, nombre, telefono) VALUES (?, ?, ?)', [id, nombre, telefono]);
        res.status(201).json({ message: 'cliente creado', id });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.put('/cliente/:id', async (req, res) => {
    try {
        const { nombre, telefono } = req.body;
        await pool.query('UPDATE cliente SET nombre=?, telefono=? WHERE id_cliente=?', [nombre, telefono, req.params.id]);
        res.json({ message: 'Cliente actualizado' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get('/pelicula/:id_pelicula', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM pelicula WHERE id_pelicula=?', [req.params.id_pelicula]);
        if (rows.length === 0) return res.status(404).json({ message: 'Pelicula no encontrada' });
        res.json(rows[0]);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/pelicula', async (req, res) => {
    try {
        const { nombre_pelicula, precio } = req.body;
        const id_pelicula = crypto.randomUUID();
        await pool.query('INSERT INTO pelicula (id_pelicula, nombre_pelicula, precio) VALUES (?, ?, ?)', [id_pelicula, nombre_pelicula, precio]);
        res.status(201).json({ message: 'pelicula creada', id: id_pelicula });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

app.put('/pelicula/:id_pelicula', async (req, res) => {
    try {
        const { nombre_pelicula, precio } = req.body;
        await pool.query('UPDATE pelicula SET nombre_pelicula=?, precio=? WHERE id_pelicula=?', [nombre_pelicula, precio, req.params.id_pelicula]);
...
app.delete('/pelicula/:id_pelicula', async (req, res) => {
    try {
        await pool.query('DELETE FROM pelicula WHERE id_pelicula=?', [req.params.id_pelicula]);
