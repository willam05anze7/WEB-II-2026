...
const CrearFila = (nombre_pelicula, precio, id_pelicula) => {
    const fila = document.createElement('tr');
    const contenido = `
        <td class="td" data-td>${nombre_pelicula}</td>
        <td>${precio}</td>
        <td>
            <ul class="table__button-control">
                <li>
                    <a href="../screens/editar_producto.html?id=${id_pelicula}"
                    class="simple-button simple-button--edit">
                    Editar
                    </a>
                </li>
                <li>
                    <button class="simple-button simple-button--delete" type="button" id="${id_pelicula}">
                    Eliminar
                    </button>
                </li>
            </ul>
    ...
    btn.addEventListener("click", () => {
        if (confirm("¿Estás seguro de eliminar este producto?")) {
            productoService.eliminar_productos(id_pelicula)
                .then(() => {
                    alert("Producto eliminado");
                    window.location.reload();
                })
...
if (tabla) {
    productoService.listar_productos()
        .then((data) => {
            ...
            data.forEach(({ nombre_pelicula, precio,  id_pelicula }) => {
                const nueva_fila = CrearFila(nombre_pelicula, precio, id_pelicula);
                tabla.appendChild(nueva_fila);
            });
        })
