...
const CrearFila = (nombre, email, id) => {
    ...
    const btn = fila.querySelector("button");
    btn.addEventListener("click", () => {
        const id = btn.id; // ✅ sin link de Notion
        clienteService.eliminar_cliente(id)
        .then(respuesta => {
            alert("cliente eliminado");
...
clienteService.listar_clientes()
    .then((data) => {
        data.forEach(({ Nombre, email, ID_cliente }) => {
            const nueva_fila = CrearFila(Nombre, email, ID_cliente);
            tabla.appendChild(nueva_fila);
        });
    }).catch((error) => alert("ERROR"));
...
// clienteService.listar_clientes()
//     .then((data) => {
//         data.forEach(({Nombre, email, ID_cliente}) => { // ✅ mayúsculas como en la BD
//             const nueva_fila = CrearFila(Nombre, email, ID_cliente);
//             tabla.appendChild(nueva_fila);
//         });
//     }).catch((error) => alert("ERROR"));